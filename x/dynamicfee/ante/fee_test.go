...
func TestAnteHandle(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(m mocks) {
				...
				m.DynamicfeeKeeper.EXPECT().GetMinGasPrice(m.ctx, types.DefaultFeeDenom).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fmd := ante.NewDynamicfeeCheckDecorator(m.AccountKeeper, m.BankKeeper, feeGrantKeeper, m.DynamicfeeKeeper, nil)
			if tt.genTx {
